...
void scheduler(std::vector<hpx::naming::id_type> workqueues,
  ...
  while (running) {
    ...
    task = hpx::async<workstealing::priorityworkqueue::steal_action>(local_workqueue).get();
    if (task) { perf_localSteals++; }
    if (distributed && !task) {
      if (last_remote != here) {
        task = hpx::async<workstealing::priorityworkqueue::steal_action>(last_remote).get();
        if (task) {
          perf_distSteals++;
        }
      }
      ...
      if (!task) {
        ...
        task = hpx::async<workstealing::priorityworkqueue::steal_action>(*victim).get();
        if (task) {
          perf_distSteals++;
          last_remote = *victim;
        } else {

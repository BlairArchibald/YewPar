...
NSHivert() {
	for i in {1..5}; do
		timeout 3600 mpiexec -n $1 -f $HOSTFILE ./build/install/bin/NS-hivert -g 51 --skel budget -b 10000000 --hpx:threads 16 --scaling >> ns_hivert_budget_$1.txt
		sleep 60
		timeout 3600 mpiexec -n $1 -f $HOSTFILE ./build/install/bin/NS-hivert -g 51 --skel stacksteal --hpx:threads 16 --scaling >> ns_hivert_stack_steal_$1.txt
	done
}
